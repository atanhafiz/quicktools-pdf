...
// Strict PDF → Image conversion simulation
const processFiles = async (files, setProgress, outputName, setError, setFile, format) => {
  try {
    const file = files[0];
    ...
    // Default output name
    let defaultName = file.name.replace(/\.pdf$/i, `.${format}`);
    let finalName = outputName && outputName.trim() !== "" ? outputName : defaultName;
    if (!finalName.toLowerCase().endsWith(`.${format}`)) finalName = finalName + `.${format}`;
    ...
    return { url: URL.createObjectURL(blob), name: finalName }; // ✅ modal trigger
  } catch (err) {
...
export default function PdfToImage() {
  ...
  const handleProcess = (files, setProgress) =>
    processFiles(files, setProgress, outputName, setError, setFile, format);
    ...
    <div className="flex justify-center items-start mt-16 px-4">
      <div className="w-full max-w-xl p-6 bg-white dark:bg-gray-900 rounded-xl shadow-lg">
        ...
        <p className="text-gray-700 dark:text-gray-200 mb-6">
          Export your PDF pages into image format (PNG or JPG). Only PDF files are accepted.
        </p>
        ...
        <div className="mb-4">
          ...
          <input
            ...
            placeholder={`Enter output file name.${format}`}
        ...
        <PdfToolWrapper
          ...
          multiple={false}
          outputName={outputName || `converted.${format}`}
          accept=".pdf"
          setFile={setFile}
        />
      </div>
