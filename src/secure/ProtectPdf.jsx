...
const processFiles = async (files, setProgress, password, outputName, setError) => {
  try {
    ...
    // Default output name
    let finalName = outputName && outputName.trim() !== "" ? outputName : "protected.pdf";
    ...
    // 🔒 Password not really used (simulate only)
    // Real implementation: send to API for encryption
...
export default function ProtectPdf() {
  ...
  const handleProcess = (files, setProgress) =>
    processFiles(files, setProgress, password, outputName, setError);
      ...
      <div className="w-full max-w-xl p-6 bg-white dark:bg-gray-900 rounded-xl shadow-lg">
        ...
        <div className="mb-4">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
            Default will be <b>protected.pdf</b> if left empty.
          </p>
        </div>
        ...
        <PdfToolWrapper
          ...
          multiple={false}
          outputName={outputName || "protected.pdf"}
          accept=".pdf"
        />
      </div>
    </div>
